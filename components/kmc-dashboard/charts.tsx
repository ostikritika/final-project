...
import { kmcPrograms, thematicAreas, projectPhases } from "@/lib/kmc-data"
...
export function ThematicDistributionChart() {
  const data = thematicAreas
    .map((area) => ({
      ...
      value: kmcPrograms.filter((p) => p.thematicArea === area.id).length,
      color: area.color,
    }))
    .filter((d) => d.value > 0)
...
export function PhaseDistributionChart() {
  const data = projectPhases.map((phase) => ({
    name: phase.label,
    count: kmcPrograms.filter((p) => p.projectPhase.phase === phase.value).length,
  }))
...
export function LinkageScoreChart() {
  const avgSDG = kmcPrograms.reduce((sum, p) => sum + p.linkageScores.sdgScore, 0) / kmcPrograms.length
  const avgISO = kmcPrograms.reduce((sum, p) => sum + p.linkageScores.isoScore, 0) / kmcPrograms.length
  const avgSCI = kmcPrograms.reduce((sum, p) => sum + p.linkageScores.sciScore, 0) / kmcPrograms.length
