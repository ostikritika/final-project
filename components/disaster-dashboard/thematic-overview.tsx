...
import { disasterThematicAreas, disasterPrograms, DisasterThematicArea } from "@/lib/disaster"
import {
  Heart,
  Siren,
  Flame,
    ...
    <Card className="border border-border">
      ...
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {disasterThematicAreas.map((area: DisasterThematicArea) => {
            const filteredPrograms = disasterPrograms.filter((p) => p.thematicArea === area.id)
            const programCount = filteredPrograms.length
            const totalBudget = filteredPrograms.reduce((sum, p) => sum + (p.budget ?? 0), 0)
            const Icon = (iconMap[area.icon] || Heart) as React.ElementType
              ...
              >
                <div className="flex items-start gap-3">
                  <div
                    ...
                    style={{ backgroundColor: `${area.color}15` }} // semi-transparent
                  >
