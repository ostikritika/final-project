...
import { environmentPrograms } from "@/lib/environment-data"
import { FileText, DollarSign, Target, TrendingUp } from "lucide-react"

export function StatsCards() {
  const totalBudget = environmentPrograms.reduce((sum, p) => sum + p.budget, 0)

  const avgLinkage =
    environmentPrograms.reduce(
      (sum, p) =>
        sum +
        (p.linkageScores.sdgScore +
          ...
          3,
      0
    ) / environmentPrograms.length

  const avgProgress =
    environmentPrograms.reduce((sum, p) => sum + p.projectPhase.progress, 0) /
    environmentPrograms.length
  ...
  const stats = [
    {
      ...
      value: environmentPrograms.length,
      icon: FileText,
      color: "text-blue-600",
      bgColor: "bg-blue-50",
    },
