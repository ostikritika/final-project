...
import { environmentPrograms, thematicAreas, projectPhases } from "@/lib/environment-data"
...
export function ThematicDistributionChart() {
  const data = thematicAreas
    .map((area) => ({
      ...
      value: environmentPrograms  .filter((p) => p.thematicArea === area.id).length,
      color: area.color,
    }))
    .filter((d) => d.value > 0)
...
export function PhaseDistributionChart() {
  const data = projectPhases.map((phase) => ({
    name: phase.label,
    count: environmentPrograms.filter((p) => p.projectPhase.phase === phase.value).length,
  }))
...
export function LinkageScoreChart() {
  const avgSDG = environmentPrograms.reduce((sum, p) => sum + p.linkageScores.sdgScore, 0) / environmentPrograms.length
  const avgISO = environmentPrograms.reduce((sum, p) => sum + p.linkageScores.isoScore, 0) / environmentPrograms.length
  const avgSCI = environmentPrograms.reduce((sum, p) => sum + p.linkageScores.sciScore, 0) / environmentPrograms.length
