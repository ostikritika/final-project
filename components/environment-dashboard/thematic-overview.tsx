...
import { thematicAreas, environmentPrograms } from "@/lib/environment-data"
import { Heart, GraduationCap, Laptop, Building2, ClipboardCheck, Users, Siren, Leaf } from "lucide-react"
  ...
  return (
    <Card className="border border-border">
      <CardHeader className="pb-3">
        ...
        <p className="text-sm text-muted-foreground">Programs categorized by national and international standards</p>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {thematicAreas.map((area) => {
            const programCount = environmentPrograms.filter((p) => p.thematicArea === area.id).length
            const totalBudget = environmentPrograms
