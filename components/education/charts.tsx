...
import { educationPrograms, thematicAreas, projectPhases } from "@/lib/education-data"
...
export function ThematicDistributionChart() {
  const data = thematicAreas
    .map((area) => ({
      ...
      value: educationPrograms.filter((p) => p.thematicArea === area.id).length,
      color: area.color,
    }))
    .filter((d) => d.value > 0)
...
export function PhaseDistributionChart() {
  const data = projectPhases.map((phase) => ({
    name: phase.label,
    count: educationPrograms.filter((p) => p.projectPhase.phase === phase.value).length,
  }))
...
export function LinkageScoreChart() {
  const avgSDG = educationPrograms.reduce((sum, p) => sum + p.linkageScores.sdgScore, 0) / educationPrograms.length
  const avgISO = educationPrograms.reduce((sum, p) => sum + p.linkageScores.isoScore, 0) / educationPrograms.length
  const avgSCI = educationPrograms.reduce((sum, p) => sum + p.linkageScores.sciScore, 0) / educationPrograms.length
